...
**What went well**
- Early alignment with the PO via the WhatsApp channel kept scope tight.
...
- Test-first discipline on the backend (permissions, status transitions, payments)
  caught regressions immediately.
- Snapshotting money on the order and idempotent Stripe webhooks avoided a class
  of payment bugs by design.
...
- Production debugging was methodical: we read the **actual error** (Render logs,
  live header inspection) instead of guessing.

**What was challenging — the "works on my machine" gap**
Several issues were **invisible locally** because dev and prod take different paths:
- **Render blocks outbound SMTP** → emails timed out in production though they
...
- Django's default **`Cross-Origin-Opener-Policy: same-origin`** left the Google
  sign-in popup blank in production (Vite served the dev page without that header).
...
- Made it a rule to **test every feature on the deployed Render site**, not just
  locally — the layers that only exist in production (proxy, headers, port policy,
  static serving) need their own pass.
- Moved provider secrets to environment variables and forwarded `.env` into the
  backend container so local and prod read configuration the same way.
