...
| KES-03 | Google sign-in popup blank once deployed | **High** | Auth / Deploy | Django's default `Cross-Origin-Opener-Policy: same-origin` severed the popup↔page link; invisible locally (Vite serves the dev page without the header) | Set `SECURE_CROSS_ORIGIN_OPENER_POLICY = "same-origin-allow-popups"` |
...
| KES-13 | Duplicate cased UI components (`Button.jsx`/`button.jsx`, …) | Low | Open — portability risk on case-insensitive filesystems |
| KES-14 | Throttle 429 message is English (DRF default) | Low | Open — cooldown UI means users rarely see it |
...
the deployed environment take different code paths**, so a deployment-only layer
(Render's SMTP block, Django's response headers on the served SPA, template escaping
in real emails) stayed invisible until the feature was tested **on the live deployment**.
