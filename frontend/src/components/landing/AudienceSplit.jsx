...
// s'adressant à chaque audience avec son propre CTA.
const AUDIENCES = [
  {
    icon: Stethoscope,
  ...
  return (
    <section className="container py-10 sm:py-14">
      ...
      <p className="mt-2 max-w-xl text-sm text-muted-foreground sm:text-base">
        Que vous cherchiez un rédacteur ou que vous en soyez un, Kessia est fait pour vous.
      </p>
      <div className="mt-6 grid gap-5 md:grid-cols-2">
        {AUDIENCES.map((a) => (
          <div key={a.eyebrow} className="flex flex-col rounded-2xl border bg-card p-6 shadow-sm sm:p-8">
            <span className="inline-flex w-fit items-center gap-2 rounded-full bg-secondary px-3 py-1 text-xs font-medium text-muted-foreground">
