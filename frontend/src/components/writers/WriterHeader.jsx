...
// Scholar link and an optional contact button.
export default function WriterHeader({ writer, onContact, contacting, showContact, as: Tag = "h1" }) {
  const name = fullName(writer, "Rédacteur");
  const expertise = sectionVisible(writer, "expertise") ? writer.expertise_areas || [] : [];

  return (
    <div className="flex flex-col gap-4 sm:flex-row sm:items-start">
      <Avatar className="mx-auto size-24 shrink-0 sm:mx-0">
      ...
      <div className="min-w-0 flex-1 text-center sm:text-left">
        <div className="flex flex-wrap items-center justify-center gap-2 sm:justify-start">
          <Tag className="text-2xl font-semibold tracking-tight">{name}</Tag>
